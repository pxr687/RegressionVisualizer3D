...
def _show_table(mod, model_type, show_statsmodels, verbose, intercept,
    ...
    if show_statsmodels is True:
        print()
        _display(_Markdown('<b> Sample regression table: </b>'))
        ...
        if verbose is True and model_type == 'linear_regression':
            ...
            print()
            _display(_Math('\\Large Y = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %
                     (intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
        ...
        if verbose is True and model_type == 'poisson_regression':
            ...
            _display(_Markdown('<b>True population regression equation:</b>'))
            print()
            _display(_Math('\\Large ln(Y) = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %
                     (intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
            _display(_Markdown('Where:'))
            _display(_Markdown(f' $Y$: {outcome_variable_name}'))
        ...
        if verbose is True and model_type == 'binary_logistic_regression':
            ...
            _display(_Markdown('<b>True population regression equation:</b>'))
            print()
            _display(_Math('\\Large logit(Y) = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %
                     (intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
            _display(_Markdown('Where:'))
            _display(_Markdown(f' $Y$: {outcome_variable_name}'))
...
def model_plot(model_type='linear_regression',
    ...
    if model_type == model_types[0]:
        ...
        x = _np.outer(_np.linspace(axis_size - (2 * axis_size),
        ...
        pop_data_x = _np.random.choice(_np.linspace(
            axis_size - (2 * axis_size), axis_size, 32), size=population_size)
    ...
    if model_type == model_types[2]:
        ...
        ax1 = fig.add_subplot(121, projection='3d')
        ax1.plot_wireframe(x, y, z, color='darkred',
                           label='population logistic regression surface', alpha=surface_alpha)
        ax1.scatter(pop_data_x[pop_data_z >= 0.5], pop_data_y[pop_data_z >= 0.5], pop_data_z[pop_data_z >=
                    0.5], color='red', label=outcome_variable_name+' = 1 (population)', alpha=data_alpha)
        ax1.scatter(pop_data_x[pop_data_z < 0.5], pop_data_y[pop_data_z < 0.5], pop_data_z[pop_data_z < 0.5],
                    marker='x', color='red', label=outcome_variable_name+' = 0 (population)',  alpha=data_alpha)
        ax1.set_zticks([0, 1])
    ...
    _show_table(mod, model_type, show_statsmodels, verbose, intercept, predictor1_slope,
                predictor1_name, predictor2_slope, predictor2_name, interaction_slope, outcome_variable_name)
