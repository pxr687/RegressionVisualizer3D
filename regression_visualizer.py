...
def _set_angles(axes, view_angle = None, view_elevation = None):
    ...
    if view_angle != None and view_elevation == None :
        axes.view_init(azim= view_angle)
    if view_elevation != None and view_angle == None:
        axes.view_init(elev = view_elevation)
    if view_elevation != None and view_angle != None:
        axes.view_init(azim = view_angle, elev = view_elevation)
        
def _show_table(mod, model_type, show_statsmodels, verbose, intercept, predictor1_slope, predictor1_name, predictor2_slope, predictor2_name, interaction_slope, outcome_variable_name):

    '''Hidden function to print out the regression table and true population regression equation.'''

    # show the regression table?
    if show_statsmodels == True:
        print()
        display(_Markdown('<b> Sample regression table: </b>'))
        display(mod.summary())
        
        # print the true regression equation?
        if verbose == True and model_type == 'linear_regression':
            print()
            _display(_Markdown('<b>True population regression equation:</b>'))
            print()
            _display( _Math('\\Large Y = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %(intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
        ...
        if verbose == True and model_type == 'poisson_regression':
            print()
            _display(_Markdown('<b>True population regression equation:</b>'))
            print()
            _display( _Math('\\Large ln(Y) = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %(intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
            _display(_Markdown('Where:'))
            _display(_Markdown(f' $Y$: {outcome_variable_name}'))
            _display(_Markdown(f' $X_1$: {predictor1_name}'))
        ...
        if verbose == True and model_type == 'binary_logistic_regression':
            print()
            _display(_Markdown('<b>True population regression equation:</b>'))
            print()
            _display( _Math('\\Large logit(Y) = %g + %g  X_1 + %g  X_2 + %g  X_1 X_2 + \\epsilon' %(intercept, predictor1_slope, predictor2_slope, interaction_slope)))
            print()
            _display(_Markdown('Where:'))
            _display(_Markdown(f' $Y$: {outcome_variable_name}'))
            _display(_Markdown(f' $X_1$: {predictor1_name}'))
...
def model_plot(model_type = 'linear_regression',
    ...
    model_types = _np.array(['linear_regression', 'poisson_regression', 'binary_logistic_regression'])
    ...
    if verbose == True:
        ...
        print()
        _display(_Markdown(f'<b>3D {formatted_model_type} visualizer: </b>'))
        _display(_Markdown('The lefthand graph below shows some population data and the population regression surface for that data. (E.g. if a regression model were fit to all of the population data).'))
        _display(_Markdown('The righthand graph shows data from a sample which was randomly drawn from the population data. It also shows the sample regression surface (e.g. from a regression model fit to the sample data).'))
        
        # if the statsmodels table should be shown, let the user know where it will be displayed
        if show_statsmodels == True:
            _display(_Markdown('Beneath the graphs is the regression table (with slopes and p-values etc.) from the sample data. The true population regression equation used to generate the data is also shown.'))
    
    
    # check the model_type supplied is legal, raise error if not
    assert model_type in model_types, 'The model_type you have specified is not recognized! It should be one of:'+str(model_types)
    ...
    if model_type == model_types[0]:
        ...
        pop_data_y = _np.random.choice(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), size = population_size)
        pop_data_z = intercept + predictor1_slope*pop_data_x + predictor2_slope*pop_data_y + interaction_slope*pop_data_x*pop_data_y + _np.random.normal(0, error_sd, size = population_size)

        # plot the population regression surface and data
        fig = _plt.figure(figsize = plot_size)
        ax1 = fig.add_subplot(121, projection='3d')
        ax1.plot_wireframe(x,y,z, color = 'darkred', label = 'population linear regression surface', alpha = surface_alpha)
        ax1.scatter(pop_data_x, pop_data_y, pop_data_z, color = 'red', label = 'population data', alpha = data_alpha)
        ...
        mod = _sm.OLS(sample_df[outcome_variable_name], _sm.add_constant(sample_df[[predictor1_name, predictor2_name, predictor1_name+' * '+predictor2_name]])).fit()
        ...
        mod_interaction_slope = mod.params[predictor1_name+' * '+predictor2_name]
        
        # wireframe z values based on sample regression model parameters
        sample_z = mod_intercept + mod_predictor1_slope*x + mod_predictor2_slope*y  + mod_interaction_slope *x*y
            
        
        # plot the sample regression surface and data)
        ax2 = fig.add_subplot(122, projection='3d')
        ax2.plot_wireframe(x,y,sample_z, color = 'darkblue', label = 'sample linear regression surface', alpha = surface_alpha)
        ax2.scatter(sample_df[predictor1_name], sample_df[predictor2_name], sample_df[outcome_variable_name], color = 'blue', label = 'sample data', alpha = data_alpha)
    ...
    if model_type == model_types[1]:
        ...
        pop_data_x = _np.random.choice(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), size = population_size)
        pop_data_y = _np.random.choice(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), size = population_size)
        pop_data_z = _np.exp(intercept + predictor1_slope*pop_data_x + predictor2_slope*pop_data_y + interaction_slope*pop_data_x*pop_data_y + _np.random.normal(0, error_sd, size = population_size)).astype('int')
        
        # plot the population regression surface and data
        fig = _plt.figure(figsize = plot_size)
        ax1 = fig.add_subplot(121, projection='3d')
        ax1.plot_wireframe(x,y,z, color = 'darkred', label = 'population poisson regression surface', alpha = surface_alpha)
        ax1.scatter(pop_data_x, pop_data_y, pop_data_z, color = 'red', label = 'population data', alpha = data_alpha)
        ax1.set_zlabel(outcome_variable_name)
        ax1.set_title('Population (N = '+str(population_size)+') :')
        _plt.xlabel(predictor1_name)
        ...
        mod = _sm.GLM(sample_df[outcome_variable_name], _sm.add_constant(sample_df[[predictor1_name, predictor2_name, predictor1_name+' * '+predictor2_name]]), family=_sm.families.Poisson()).fit()
        ...
        # plot the sample regression surface and data
        ax2 = fig.add_subplot(122, projection='3d')
        ax2.plot_wireframe(x,y,sample_z, color = 'darkblue', label = 'sample poisson regression surface', alpha = surface_alpha)
        ax2.scatter(sample_df[predictor1_name], sample_df[predictor2_name], sample_df[outcome_variable_name], color = 'blue', label = 'sample data', alpha = data_alpha)
        ax2.set_zlabel(outcome_variable_name)
        ax2.set_title('Sample (n = '+str(sample_size)+') :')
        _plt.xlabel(predictor1_name)
    ...
    if model_type == model_types[2]:
        ...
        x = _np.outer(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), _np.ones(32))
        y = x.copy().T # transpose
        lin_pop_z = intercept + predictor1_slope*x + predictor2_slope*y + interaction_slope*x*y
        z = _np.exp(lin_pop_z)/(1 + _np.exp(lin_pop_z))

        # population datapoints
        pop_data_x = _np.random.choice(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), size = population_size)
        pop_data_y = _np.random.choice(_np.linspace(axis_size -(2 * axis_size), axis_size, 32), size = population_size)
        lin_pred = intercept + predictor1_slope*pop_data_x + predictor2_slope*pop_data_y + interaction_slope*pop_data_x*pop_data_y + _np.random.normal(0, error_sd, size = population_size)
        ...
        fig = _plt.figure(figsize = plot_size)
        ax1 = fig.add_subplot(121, projection='3d')
        ax1.plot_wireframe(x,y,z, color = 'darkred', label = 'population logistic regression surface', alpha = surface_alpha)
        ax1.scatter(pop_data_x[pop_data_z >= 0.5], pop_data_y[pop_data_z >= 0.5], pop_data_z[pop_data_z >= 0.5], color = 'red', label = outcome_variable_name+' = 1 (population)' , alpha = data_alpha)
        ax1.scatter(pop_data_x[pop_data_z < 0.5], pop_data_y[pop_data_z < 0.5], pop_data_z[pop_data_z < 0.5], marker = 'x', color = 'red', label = outcome_variable_name+' = 0 (population)',  alpha = data_alpha)
        ax1.set_zticks([0,1])
        ...
        # fit the regression model to the sample data
        mod = _sm.Logit(sample_df[outcome_variable_name], _sm.add_constant(sample_df[[predictor1_name, predictor2_name, predictor1_name+' * '+predictor2_name]])).fit()
        ...
        sample_z = _np.exp(mod_intercept + mod_predictor1_slope*x + mod_predictor2_slope*y  + mod_interaction_slope * x*y)/(1 + _np.exp(mod_intercept+ mod_predictor1_slope*x + mod_predictor2_slope*y  + mod_interaction_slope * x*y))
            
        
        # plot the sample regression surface and data
        ax2 = fig.add_subplot(122, projection='3d')
        ax2.plot_wireframe(x,y,sample_z, color = 'darkblue', label = 'sample logistic regression surface', alpha = surface_alpha)
        ax2.scatter(sample_df[predictor1_name][sample_df[outcome_variable_name] >= 0.5], sample_df[predictor2_name][sample_df[outcome_variable_name] >= 0.5], sample_df[outcome_variable_name][sample_df[outcome_variable_name] >= 0.5], 
        color = 'blue', label = outcome_variable_name+' = 1 (sample)' , alpha = data_alpha)
        ax2.scatter(sample_df[predictor1_name][sample_df[outcome_variable_name] < 0.5], sample_df[predictor2_name][sample_df[outcome_variable_name] < 0.5], sample_df[outcome_variable_name][sample_df[outcome_variable_name] < 0.5],
        color = 'blue', marker = 'x', label = outcome_variable_name+' = 0 (sample)', alpha = data_alpha )
    ...
    # show the regression table and true regression equation, if set to be displayed
    _show_table(mod, model_type, show_statsmodels, verbose, intercept, predictor1_slope, predictor1_name, predictor2_slope, predictor2_name, interaction_slope, outcome_variable_name)
